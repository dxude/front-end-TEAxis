...
const API_REGISTRO_PROFISSIONAL = 'https://back-end-plataforma-teaxis.onrender.com/api/v1/profissionais/registrar';
...
const Cadastro = () => {
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      if (error.code === 'auth/popup-blocked') {
        mensagemErro = '❌ Popup blocked. Permita popups no navegador.';
  ...
  const handleCadastroSubmit = async (e) => {
    ...
    // Alinhamento exato de dados planos com o Record DadosCadastroUsuario do Java
    let payload = {
    ...
    if (perfil === 'usuario') {
      payload = {
        ...payload,
        dataNascimento: detalhesUsuario.dataNascimento || null,
        // Converte as listas de volta para o padrão String esperado pelo Record do seu grupo
        tipoNeurodivergencia: detalhesUsuario.tipoNeurodivergencia.length > 0 ? detalhesUsuario.tipoNeurodivergencia.join(', ') : "Não informado",
        hobbies: detalhesUsuario.hobbies.length > 0 ? detalhesUsuario.hobbies : ["Geral"], // Set<String> do Java
        tipo: "usuario",
        genero: detalhesUsuario.genero || "Não informado",
        cidade: detalhesUsuario.cidade || "Não informado",
        estado: detalhesUsuario.estado || "Não informado",
        preferenciasSensoriais: detalhesUsuario.preferenciasSensoriais.length > 0 ? detalhesUsuario.preferenciasSensoriais.join(', ') : "Nenhuma",
        modoComunicacao: detalhesUsuario.modoComunicacao.length > 0 ? detalhesUsuario.modoComunicacao.join(', ') : "Padrão",
        historicoEscolar: detalhesUsuario.historicoEscolar || "Nenhum"
      };
    } else {
      payload = {
        ...payload,
    ...
    try {
      ...
      console.log('Enviando dados estruturados para o Java:', payload);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Falha ao criar o cadastro. Verifique os dados.");
      }
    ...
    return (
        <form className="cadastro-form auth-card" onSubmit={onSubmit}>
            ...
            <div className="form-actions">
                ...
                <button type="submit" className="btn btn-primary-green btn-full" disabled={loadingSubmit}>
                    {loadingSubmit ? 'Registrando...' : (currentStep === 1 ? 'Continuar' : 'Registrar e concluir')}
                </button>
            </div>
